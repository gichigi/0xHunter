...
export function validateQuery(query: string): { valid: boolean; error?: string } {
  // Always check length
  if (query.length < 3) {
    return { valid: false, error: "The Hunter needs more to track. Query must be at least 3 characters." }
  }
  if (query.length > 500) {
  ...
  if (addressMatches.length > 0) {
    ...
    for (const address of addressMatches) {
      if (!isAddress(address)) {
        return { 
          valid: false, 
          error: `The Hunter cannot track this target - invalid address format: ${address.slice(0, 10)}...` 
        }
      }
    }
  }
